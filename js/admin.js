...
/* ═══════════════════════════════════════════════
   ...
   ─ Admin generates a code per tier.
   ─ The code's SHA-256 hash is saved to localStorage.
   ─ Share the plaintext code with students who paid.
   ─ Student enters code on the locked module page.
═══════════════════════════════════════════════ */

function randomCode(tierKey) {
  const abbr    = tierKey.slice(0, 4).toUpperCase();
  const charset = 'ABCDEFGHJKLMNPQRSTUVWXYZ23456789'; /* no 0/O/1/I to avoid confusion */
  const part    = Array.from({ length: 6 }, () => charset[Math.floor(Math.random() * charset.length)]).join('');
  return `TECHG-${abbr}-${part}`;
}
...
async function generateEnrollmentCode(tierKey) {
  const code = randomCode(tierKey);
  const hash = await sha256(code);
  localStorage.setItem(`tg_grant_hash_${tierKey}`, hash);
  localStorage.setItem(`tg_grant_plain_${tierKey}`, code); /* admin-only plaintext */
  renderEnrollmentSection();
  showToast(`New code generated for ${TIERS[tierKey].label}`);
}

function renderEnrollmentSection() {
  const container = document.getElementById('enrollment-section');
  if (!container) return;

  container.innerHTML = Object.entries(TIERS).map(([key, tier]) => {
    const code     = localStorage.getItem(`tg_grant_plain_${key}`) || '— not generated yet —';
    const hasCode  = code !== '— not generated yet —';
    const enrolled = Object.keys(localStorage).filter(k => k === `tg_enrolled_${key}`).length;

    return `
      <div class="enroll-row">
        <div class="enroll-tier-info">
          <span class="enroll-tier-pill" style="background:${tier.pill};">${tier.label}</span>
          <span class="enroll-tier-class">${tier.class}</span>
        </div>
        <div class="enroll-code-wrap">
          <code class="enroll-code-display ${hasCode ? '' : 'empty'}" id="ecode-${key}">${code}</code>
          ${hasCode ? `
            <button class="enroll-copy-btn" onclick="copyEnrollCode('${key}')" title="Copy code">
              <i class="fas fa-copy"></i>
            </button>` : ''}
        </div>
        <button class="btn enroll-gen-btn" onclick="generateEnrollmentCode('${key}')">
          <i class="fas fa-redo"></i> ${hasCode ? 'Regenerate' : 'Generate Code'}
        </button>
      </div>`;
  }).join('');
}

function copyEnrollCode(tierKey) {
  const code = localStorage.getItem(`tg_grant_plain_${tierKey}`);
  if (!code) return;
  navigator.clipboard.writeText(code).then(() => {
    showToast(`Code for ${TIERS[tierKey].label} copied — send to student via WhatsApp or email`);
  }).catch(() => {
    showToast('Code: ' + code);
  });
}
...
/* ═══════════════════════════════════════════════
   FIREBASE STATUS
   Updates the fb-status badge in the navbar and
   (on first admin load) syncs Firestore → localStorage
   so remote content is available locally too.
═══════════════════════════════════════════════ */

async function initFirebase() {
  const db = getDb(); /* from firebase-config.js */
  if (!db) {
    _setFbStatus('not-configured');
    return;
  }
  _setFbStatus('connecting');
  try {
    const remote = await fsReadAll();
    if (remote && Object.keys(remote).length) {
      /* Merge remote into localStorage so post.js can read it offline too */
      const merged = { ...getPosts(), ...remote };
      localStorage.setItem('tg_posts', JSON.stringify(merged));
      Object.entries(remote).forEach(([k, v]) =>
        localStorage.setItem(`tg_post_${k}`, JSON.stringify(v))
      );
      renderModuleList();
      renderPostsTable();
    }
...
function _setFbStatus(status) {
  const el = document.getElementById('fb-status');
  if (!el) return;
  const map = {
    'not-configured': { color: '#f59e0b', icon: 'fa-exclamation-triangle', label: 'Setup Firebase' },
    connecting:       { color: '#60a5fa', icon: 'fa-spinner fa-spin',      label: 'Connecting…'    },
    ok:               { color: '#4ade80', icon: 'fa-cloud',                 label: 'Firestore live' },
    error:            { color: '#f87171', icon: 'fa-exclamation-circle',    label: 'DB error'       },
  };
...
function renderModuleList() {
  ...
  document.getElementById('admin-module-list').innerHTML = modules.map((mod, i) => {
    const key        = `${currentTier}_${i}`;
    const fromAdmin  = !!posts[key];
    const fromSeed   = typeof SEED_POSTS !== 'undefined' && !!SEED_POSTS[key];
    const hasContent = fromAdmin || fromSeed;
    const dotTitle   = fromAdmin ? 'Has content (saved)' : fromSeed ? 'Has content (seeded)' : 'Empty';
    return `
    <div class="admin-module-item${i === currentModuleIdx ? ' active' : ''}${hasContent ? ' has-content' : ''}"
      ...
      <span class="admin-mod-dot" title="${dotTitle}"></span>
    </div>`;
  }).join('');
}
...
function selectModule(idx) {
  ...
  const adminSaved = getPosts()[postKey];
  const seedData   = (typeof SEED_POSTS !== 'undefined' && SEED_POSTS[postKey]) || null;
  const postData   = adminSaved || seedData || { content: '', status: 'draft', updatedAt: null };
...
async function savePost(status) {
  ...
  /* Also persist to Firestore (no-op if Firebase not configured) */
  const saved = await fsWrite(postKey, posts[postKey]);
  if (saved) _setFbStatus('ok');
}
...
document.getElementById('import-file-input').addEventListener('change', e => {
  ...
  reader.onload = async ev => {
    try {
      ...
      /* Push each imported post to Firestore (no-op if not configured) */
      await Promise.all(Object.entries(data).map(([k, v]) => fsWrite(k, v)));
      renderModuleList();
      renderPostsTable();
...
function deletePost(key) {
  ...
  fsDelete(key); /* remove from Firestore too (no-op if not configured) */
  renderModuleList();
  renderPostsTable();
