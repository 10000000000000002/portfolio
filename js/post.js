...
if (!tier || !TIERS[tier] || isNaN(modIdx) || modIdx < 0 || modIdx >= TIERS[tier].modules.length) {
  ...
  function loadContent() {
    ...
    /* Admin-published content takes priority; fall back to seeded content */
    const stored   = safeGetJSON(`tg_post_${postKey}`);
    const postData = stored
      ?? ((typeof SEED_POSTS !== 'undefined' && SEED_POSTS[postKey]) || null);
    ...
    if (!postData) {
      ...
      /* hide lang selector when no content */
  ...
  function showLocked() {
    document.getElementById('post-content-area').innerHTML = `
      <div class="locked-overlay">
        ...
        <p>Enroll in the full <strong>${tierData.label}</strong> course to access all modules and the hands-on project.</p>
        <a href="courses.html?enroll=${tier}" class="btn btn-primary">
          <i class="fas fa-bolt"></i> Enroll Now — ${tierData.price}
        </a>
      </div>`;
    document.getElementById('rating-section').style.display = 'none';
  }
